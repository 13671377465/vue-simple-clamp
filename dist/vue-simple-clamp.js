(function (global, factory) {
  ...
  var upperTwo = function upperTwo() {
    return toUpper(arguments.length <= 2 ? undefined : arguments[2]);
  };

  var setUpperCase = ifElse(test(reg), replace(reg, upperTwo), identity);

  var polyfillGetStyleProp = function polyfillGetStyleProp(el, prop$$1) {
    var p = compose(setUpperCase, setFloatProperty)(prop$$1);
    return el.currentStyle && el.currentStyle[p] ? el.currentStyle[p] : null;
  };

  var getPropertyValue = function getPropertyValue(style) {
    return style.getPropertyValue.bind(style);
  };

  var winGetComputedStyle = function winGetComputedStyle(el) {
    return window.getComputedStyle(el, null);
  };

  var hasGetComputedStyle = function hasGetComputedStyle() {
    return has('getComputedStyle', window);
  };

  var curryPolyfillGetStyleProp = function curryPolyfillGetStyleProp() {
    for (var _len = arguments.length, arg = new Array(_len), _key = 0; _key < _len; _key++) {
      arg[_key] = arguments[_key];
    }

    return curry(polyfillGetStyleProp)(arg);
  };
  /**
   * 返回元素的当前样式
  ...
  var computeStyle = ifElse(hasGetComputedStyle, compose(getPropertyValue, winGetComputedStyle), curryPolyfillGetStyleProp);
  /**
   * @param {HTMLElement} el
   * @return {number} 整数
   * 得到元素的行高
  ...
  function getLineHeight(el) {
    var getElStyle = computeStyle(el);
    var elLineHeight = getElStyle('line-height');
    return equals(elLineHeight, 'normal') ? multiply(parseInt(getElStyle('font-size')), 1.4) : parseInt(elLineHeight);
  }
  /**
   * @param {HTMLElement} el
  ...
  var getMaxHeight = function getMaxHeight(el, clmp) {
    return multiply(getLineHeight(el), clmp);
  };
  /**
   * @param {HTMLElement} el
  ...
  function getMaxLines(el, height) {
    var getLines = compose(Math.floor, divide);
    var defaultToHeight = defaultTo(el.clientHeight);
    var availHeight = defaultToHeight(height);
  ...
  var hookClamp = function hookClamp(el, binding) {
    clamp$1(el, binding.value);
  };
  ...
  return vueSimpleClamp;

}));
//# sourceMappingURL=vue-simple-clamp.js.map
